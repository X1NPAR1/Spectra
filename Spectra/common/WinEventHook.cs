    ...
    class WinEventHook
    {
        ...
        public void RemoveWinEventHook()
        {
            try
            {
                ...
                if (!result)
                {
                    MainForm.Log(new Exception("UnhookWinEvent(winEventHookHandle) failed. winEventHookHandle = " + _winEventHookHandle));
                }
            }
            catch (Exception ex)
            {
                MainForm.Log(new Exception("UnhookWinEvent(winEventHookHandle) failed."));
            }
            finally
            {
